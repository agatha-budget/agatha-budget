...
mkShell {
buildInputs = [
kotlin postgresql_15 gradle_9 temurin-bin-17 python3 go-task
];
...
psql -U postgres -tc "SELECT 1 FROM pg_database WHERE datname = 'agatest'" | grep -q 1 || psql -U postgres -c "CREATE DATABASE agatest"
'';
}
