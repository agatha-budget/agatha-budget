...
open class AccountDaoTest : ITest {
    ...
    fun getAccountWithAmountForBudget() {
        ...
        val operation1 = Operation(account.id, TestData.feb_02_2021, Category.INCOME_ID, 15065, 1, "mémo")
        val operation2 = Operation(account.id, TestData.march_02_2021, Category.INCOME_ID, -10000, 2, "reimbursement")
        operationDao.insert(operation1)
        operationDao.insert(operation2)

        val account2 = Account("account2", budget.id)
        accountDao.insert(account2)
        val operation3 = Operation(account2.id, TestData.feb_02_2021, Category.INCOME_ID, 7777, 3, "mémo")
    ...
    fun getAccountWithNullAmountForBudget() {
        ...
        val account = Account("account", budget.id)
        accountDao.insert(account)
        val operation1 = Operation(account.id, TestData.feb_02_2021, Category.INCOME_ID, 10000, 1, "mémo")
        val operation2 = Operation(account.id, TestData.march_02_2021, Category.INCOME_ID, -10000, 2, "reimbursement")
        operationDao.insert(operation1)
        operationDao.insert(operation2)
    ...
    fun getAccountWithNegativeAmountForBudget() {
        ...
        accountDao.insert(account)
        val operation1 = Operation(account.id, TestData.feb_02_2021, Category.INCOME_ID, 10000, 1, "mémo")
        val operation2 = Operation(account.id, TestData.march_02_2021, Category.INCOME_ID, -15000, 2, "reimbursement")
        operationDao.insert(operation1)
        operationDao.insert(operation2)
